        ...
        route.route("/downloads") {
            ...
            get {
                ...
                call.respond(toJsonElement(mapOf(
                    ...
                    // 启动期配置改过但引擎正忙、还没重启：UI 据此提示"下次引擎启动生效"
                    "pending_engine_restart" to downloadManager.pendingEngineRestart
                )))
            }
            ...
            post("/{id}/rename") {
                val id = call.parameters["id"] ?: ""
                val body = call.receiveJsonObject()
                val newName = body["name"]?.jsonPrimitive?.contentOrNull ?: ""
                val ok = downloadManager.renameTask(id, newName)
                if (ok) call.respond(toJsonElement(mapOf("success" to true)))
                else call.respondFail(HttpStatusCode.BadRequest, ErrorCode.OPERATION_FAILED, "重命名失败")
            }
            ...
            get("/validate-path") {
                ...
                if (path.isBlank()) {
                    call.respondFail(
                        HttpStatusCode.BadRequest, ErrorCode.BAD_REQUEST, "path is required",
