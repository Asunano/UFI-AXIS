 ...
 *
 * ## 边界（重要）
 * 限的是 **core → 本机 shell** 的并发，**不是客户端 → core 的请求数**。`/api` 下的接口没有请求级
 * 限流（见 `HttpServer` 的 `/api` 块注释），所以这里只提供**阻塞等许可**的执行方法：
 * 排队会让接口慢一点，但不会把限流变成 429 或空数据。
 *
 * ## 核心功能
 * - **自适应限流**: root shell 默认 5 并发（2~6 动态调整），普通 shell 5 并发（2~8）
...
object ShellQoS {
    ...
    private const val DEFAULT_ROOT_PERMITS = 5  // ADB shell 无 sprd_ipc_probe 竞态，可放宽并发
    ...
    // 预编译 Regex — 避免 normalizeCommand() 每次调用都重新编译
    private val TIMESTAMP_REGEX = Regex("_=\\d+")
    private val WHITESPACE_REGEX = Regex("\\s+")

    private val rootSemaphore = AdaptiveSemaphore(
        initialPermits = DEFAULT_ROOT_PERMITS, minPermits = 2, maxPermits = 6
    )
    ...
    fun updateCacheTtl(ttlMs: Long) {
        ...
        // 2026-08-28：只在真的变化时打日志。调用方是 30s 一次的性能监控循环，
        // 稳态下 TTL 恒定不变，原来无条件打 INFO ≈ 每天 2880 条无用日志。
    ...
    fun updateRootPermits(permits: Int) {
        val clamped = permits.coerceIn(1, 6)
    ...
    private fun normalizeCommand(cmd: String): String {
        return cmd.replace(TIMESTAMP_REGEX, "")  // 去除 _=timestamp
     ...
     * 每条命令包装 timeout 5s，防止单条 hang（如 cat FIFO）阻塞整批。
     */
    private fun buildBatchScript(commands: List<String>, separator: String): String {
        val sb = StringBuilder()
        commands.forEachIndexed { index, cmd ->
            if (index > 0) sb.append(" ; echo \"$separator\" ; ")
            // 单命令 5s 超时保护 — 防止一条命令 hang 影响整批
            sb.append("timeout 5 sh -c '").append(cmd.replace("'", "'\\''")).append("'")
        }
