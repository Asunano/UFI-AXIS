...
class TelephonyCollector(private val context: Context) {
    ...
    // 模块级公开 API（被 :core 的 DataScheduler 引用）
    ...
    /**
     * 设备当前是否**真的能上外网**（`connectivity` 告警的判定依据，2026-09-04 改）。
     *
     * 与 [isNetworkRegistered] 的区别是本质性的：
     * - [isNetworkRegistered] 只看 `networkOperator` 非空，即**modem 注册到了运营商**。
     *   欠费停机、PPP 没拨上、上游 WiFi 中继掉线时它照样返回 true —— 于是「设备没网」
     *   报不出来；而它在切换制式/重新搜网的瞬间会短暂变空，反倒把**网络抖动**报成断网。
     *   用户实测的「时不时抖一下就触发」「关掉 WiFi 用移动数据也触发」都出自这里。
     * - 本方法用 [NetworkCapabilities.NET_CAPABILITY_VALIDATED]：系统实际探测过外网可达才为真，
     *   且**与承载方式无关** —— 蜂窝换 WiFi 中继只要外网还通就不算状态变化。
     *
     * VALIDATED 在个别 ROM 上可能长期为 false（无探测通道），因此回退到
     * [NetworkCapabilities.NET_CAPABILITY_INTERNET]：宁可少报也不能因为取不到能力位就恒判离线。
     */
    fun isDeviceOnline(): Boolean {
        return try {
            ...
            // 取不到能力位时按「在线」处理：这里是告警判定的输入，误报离线比漏报更烦人
            true
        }
    }
