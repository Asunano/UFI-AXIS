...
class TrackerManager(
    ...
    companion object {
        ...
        /**
         * 内置兜底 Tracker（逗号分隔）。远程列表还没拉到（首次安装）或拉取失败时，
         * aria2 至少还有 tracker 可用 —— 否则磁链只能靠 DHT，实测会一直停在 metadata 阶段。
         */
        const val BUILTIN_TRACKERS: String =
    ...
    /**
     * 拉取成功后的回调（参数为合并后的完整列表）。DownloadManager 用它把新列表
     * 热加载进正在运行的 aria2 —— 定时刷新原来只写文件，运行中的 aria2 要等下次重启才看得到。
     */
    @Volatile
    var onTrackersUpdated: ((String) -> Unit)? = null
