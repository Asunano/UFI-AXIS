...
class ModelsTest {
    ...
    fun `WebSocketMessage - create`() {
        val data = JsonObject(mapOf("usage_percent" to JsonPrimitive(45.2)))
        ...
        val usage = (message.data as? JsonObject)
            ?.get("usage_percent")?.jsonPrimitive?.content?.toDouble()
        assertEquals(45.2, usage ?: 0.0, 0.01)
    }
}
