<template>
  <span class="file-icon-wrap" :class="`k-${kind}`" :title="kindLabel">
    ...
    <svg v-else-if="kind === 'archive'" viewBox="0 0 24 24" class="ficon" aria-hidden="true">
      <path
        fill="currentColor"
        d="M20 6h-8l-2-2H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm-6 8h-2v-1h-1v1h-2v-1h-1v1H6v-1H5v4h14v-4h-5v1h-1v-1zm1-3h-2V7h2v4z"
      />
    </svg>
    <!-- APK -->
    ...
    <svg v-else-if="kind === 'doc'" viewBox="0 0 24 24" class="ficon" aria-hidden="true">
      <path
        fill="currentColor"
        d="M18 2H6c-1.1 0-2 .9-2 2v16c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zM9 4h2v5l-1 .75L9 9V4zm9 16H6V4h1v9h5v7h6z"
      />
    </svg>
    <!-- 未知文件 -->
...
/* 各文件类型一色：这正是 --cat-*（分类色）的用途 —— 按色相取，不按业务名。
   2026-09-15 之前这里是 7 个只有浅色档的 hex，暗色下深字压在深色卡片上偏暗；
   --cat-*-fg 自带暗色档且对比度已按 WCAG 验算（见 main.css）。 */
