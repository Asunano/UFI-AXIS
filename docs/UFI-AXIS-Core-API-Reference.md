...
| rule\_*      | —      | 命中规则的**快照**，刻意冗余：规则删掉之后这条记录仍要读得懂                                                       |
...
> 三个接口均带 zip-slip / 路径穿越防护：目标路径须经 `safeResolve` 规范化，且必须落在  
> `/sdcard`、`/storage`、`/mnt/media_rw` 三个用户存储根之一（[isUserStoragePath]）。  
> 解压时对每个条目做规范化 + 规范路径越界检查，拒绝 `..` 与符号链接逃逸。

#### `POST /api/files/extract`

解压受支持的压缩包。支持 `zip` / `tar.gz` / `tgz` / `tar` / `gz`；  
`rar` / `7z` / `bz2` / `xz` 后端暂不支持，会返回 `400`。

**请求体：**

```json
{ "path": "/storage/emulated/0/archive.zip", "destination": "/storage/emulated/0/archive" }
```

- `path` — 压缩包路径（必填）
- `destination` — 解压目标目录（可选）。缺省时默认为 `<源文件父目录>/<源文件名去扩展名>`。

**响应：**

```json
{ "success": true, "destination": "/storage/emulated/0/archive", "kind": "zip" }
```

- `kind` — 归档类型：`zip` / `tgz` / `tar` / `gz`
- `destination` — 实际解压目录（`.gz` 为解压出的文件所在目录）
...
```json
{ "paths": ["/storage/emulated/0/a.txt", "/storage/emulated/0/docs"], "destination": "/storage/emulated/0/a.zip" }
```

- `paths` — 待压缩的源路径列表（必填，至少 1 个）
- `destination` — 输出 `.zip` 路径（可选）。缺省时默认为 `<首个源父目录>/<首个源文件名去扩展名>.zip`。

**约束：** 目标已存在、或目标路径等于任一源路径时返回失败（避免覆盖/自包含损坏）。

**响应：**

```json
{ "success": true, "path": "/storage/emulated/0/a.zip", "size": 12345 }
```

#### `POST /api/files/checksum`

计算文件摘要（流式读取，不会把整文件读进内存）。

**请求体：**

```json
{ "path": "/storage/emulated/0/a.txt", "algorithms": ["md5", "sha256"] }
```

- `path` — 文件路径（必填）
- `algorithms` — 算法列表（可选）。缺省 `["md5","sha1","sha256"]`；支持 `md5` / `sha1` / `sha256` / `sha512`。
...
```json
{
  ...
  "algorithms": { "md5": "d41d8c…", "sha1": "da39a3…", "sha256": "e3b0c4…" }
}
```
